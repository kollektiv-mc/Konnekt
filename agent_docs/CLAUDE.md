...
- **Icons come from `components/ui/Icon.tsx`**, never a literal glyph (`×`, `⚙`,
  `✓`) and never a hand-pasted `<svg>`. The glyph itself is passed as a
  component, sourced from `frontend/src/lib/icons.ts` — the only module in the
  app allowed to import `lucide-react`, so the icon set is swappable from one
  file and `pnpm check-bundle` has one place to read. Adding an icon is a name
  added to that file's import and export, taken from https://lucide.dev/icons.
  ...
  `applySkin()` retheming included. Stroke weight is one screen value,
  ...
  next sync. An icon inside a button stays `aria-hidden` and the *button*
  carries the `aria-label`; `Icon`'s `label` prop is for the rare icon that
  carries meaning alone (see `ActiveProcesses`'s outcome glyph).
