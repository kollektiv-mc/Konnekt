...
- **Chrome icons are SVG, tile icons are text.** A control's icon comes from
  `components/ui/icons.tsx` and is rendered inside `components/ui/IconButton`,
  which is the one square box the whole navbar column shares. Do not reach for a
  Unicode glyph: a font positions each one by its own metrics, so ⚙ ✎ ⤢ × could
  not be made to line up with each other under any amount of padding, and some
  of them resolve to an emoji font on one platform and a symbol font on the
  next. `TILE_REGISTRY`'s `icon` strings are the deliberate exception — those
  are ASCII marks in a monospace column, not controls.
