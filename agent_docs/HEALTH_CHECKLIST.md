...
must find nothing. `text uses the type scale` is the same shape for `text-[Npx]`
over the whole of `frontend/src`, widened from the scheduler once #368 swept
the rest (HEALTH_LOG, 2026-09-18), with one file excluded by name and the
reason beside the literal. `scheduler colours live in blockMeta` is scoped to
`frontend/src/tiles/scheduler` because that is as far as that sweep has got
(#163). Read an entry's `diagnosis` before judging a match; the rule is about
the token layer, not the regex.
...
**P3 — Editable fields disagree about their background** (filed 2026-09-17)

Most of the app's fields are `bg-hover`: `LayoutPresets`, both `CommandDialogs`
inputs, `SettingsModal`. Two are `bg-surface`: the scheduler's `FIELD_CLASS` and
`ServerEditForm`'s `inputClass`. Four to two is a majority, not a rule, and
nothing in the token names says which is meant, since `hover-surface` is named
for where it came from rather than for the role it is doing here.

#163 left both as they are rather than picking a side. The shared `Combobox`
backs the scheduler's preset fields *and* the server edit form's loader picker,
so moving it to `bg-hover` would have made that dropdown lighter than the
inputs beside it: one form made consistent by splitting another. The hover and
focus border went in, because that one is additive and both consumers wanted
it.

Settling this means deciding which background an editable field has, saying so
where the token is defined, and moving whichever side loses. It spans
`area:scheduler` and `area:server-setup`, so it is its own change rather than a
rider on either.
Filed as #385 (2026-09-18).
