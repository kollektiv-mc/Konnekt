...
// that panics is contained (the emitter and the other handlers never see it)
// and logged: one line in konnekt.log naming the event, because a scheduler
// trigger that panics on an event otherwise just stops firing with no trace
// anywhere (#261).
