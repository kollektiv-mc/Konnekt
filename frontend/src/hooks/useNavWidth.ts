...
export function useNavWidth() {
  ...
  const [clamped, setClamped] = useState(() => clampNavWidth(stored, viewportWidth()))

  const storedRef = useRef(stored)
  storedRef.current = stored

  useEffect(() => {
    setClamped(clampNavWidth(stored, viewportWidth()))
  }, [stored])
  ...
  useEffect(() => {
    const onResize = () => setClamped(clampNavWidth(storedRef.current, viewportWidth()))
    window.addEventListener('resize', onResize)
    return () => window.removeEventListener('resize', onResize)
  }, [])

  const width = dragWidth ?? clamped
